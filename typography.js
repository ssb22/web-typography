...
if(document.getElementsByTagName && navigator.userAgent.indexOf("Googlebot/")==-1) {
  ...
  typefix = function(str) {
    ...
    if (supports_spacing) str=str.replace(/([A-Za-z][A-Za-z][)]?(<!--.*?-->[)]?)*[.?!][)\u2019\u201d]*(<!--.*?-->)*[)\u2019\u201d]*)\s+((<!--.*?-->\s*)*[^A-Za-z0-9]*[A-Z])/g,"$1\u2002"+(supports_0space?"\u200B":"")+"$4"); // use en-space between sentences (must be after quote substitution above), plus zero-width space (if supported) to confirm this is a breakpoint (as some versions of at least Webkit don't break at en-space by default)
